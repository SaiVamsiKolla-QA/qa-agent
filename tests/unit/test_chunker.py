...
def test_chunker_empty_input_returns_empty_list() -> None:
    ...
    assert chunk_texts([], chunk_size=500, chunk_overlap=100, source_doc="test.pdf") == []
    assert chunk_texts(
        _pages(["", "   "]), chunk_size=500, chunk_overlap=100, source_doc="test.pdf"
    ) == []
...
def test_chunker_raises_value_error_when_overlap_equals_chunk_size() -> None:
    ...
    with pytest.raises(ValueError):
        chunk_texts(_pages(["some text here"]), chunk_size=5, chunk_overlap=5, source_doc="test.pdf")


def test_chunker_raises_value_error_when_overlap_exceeds_chunk_size() -> None:
    """chunk_overlap > chunk_size must raise ValueError."""
    with pytest.raises(ValueError):
        chunk_texts(_pages(["some text here"]), chunk_size=5, chunk_overlap=6, source_doc="test.pdf")


def test_chunker_result_dicts_have_expected_keys() -> None:
    """Each chunk dict has exactly the keys: text, source_doc, page, chunk_index, chunk_id."""
    pages = _pages(["alpha beta gamma delta epsilon"])

    result = chunk_texts(pages, chunk_size=10, chunk_overlap=2, source_doc="doc.pdf")

    assert len(result) == 1
    assert set(result[0].keys()) == {"text", "source_doc", "page", "chunk_index", "chunk_id"}
...
def test_chunker_page_number_reflects_chunk_start_page() -> None:
    ...
    pages = [{"page": 1, "text": "alpha beta gamma"}, {"page": 2, "text": "delta epsilon zeta"}]
