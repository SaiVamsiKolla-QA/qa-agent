...
- [x] `llm_client.py` logs `tokens_in=<word_count>` but the value is a word count, not a real token count. The label is misleading for anyone reading the logs. (fixed: renamed to words_in)
